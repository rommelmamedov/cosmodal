...
import { ConnectedWallet, ConnectWalletToChainFunction, IWalletManagerContext, UseWalletResponse, WalletConnectionStatus } from '../types';
import { getChainInfo, getConnectedWalletInfo } from '../utils';

export const WalletManagerContext = createContext<IWalletManagerContext | null>(null);
...
export const useWallet = (chainId?: ChainInfo['chainId']): UseWalletResponse => {
	const {
	...
	// established.
	const shouldConnectToChainId = managerStatus === WalletConnectionStatus.Connected && !!managerConnectedWallet && !!chainId;
	const [chainIdStatus, setChainIdStatus] = useState<WalletConnectionStatus>(WalletConnectionStatus.Initializing);

	const [chainIdError, setChainIdError] = useState<unknown>();
	const [chainIdConnectedWallet, setChainIdConnectedWallet] = useState<ConnectedWallet>();
	useEffect(() => {
...
export const useConnectWalletToChain = () => {
	const { status, connectedWallet, chainInfoOverrides, getSigningCosmWasmClientOptions, getSigningStargateClientOptions } =
		useWalletManager();
