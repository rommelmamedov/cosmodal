...
import React, { FunctionComponent, PropsWithChildren, ReactNode, useCallback, useEffect, useMemo, useRef, useState } from 'react';
...
export const WalletManagerProvider: FunctionComponent<WalletManagerProviderProps> = ({
	children,
	enabledWalletTypes,
	defaultChainId,
	chainInfoOverrides,
	...
	const [status, setStatus] = useState<WalletConnectionStatus>(WalletConnectionStatus.Initializing);
	...
	const [connectToWalletUponReset, setConnectToWalletUponReset] = useState<Wallet>();
	...
	useEffect(() => {
		...
		import('@keplr-wallet/stores')
			...
			.then(keplr => keplr && keplr.mode === 'mobile-web' && setIsEmbeddedKeplrMobileWeb(true))
	...
	useEffect(() => {
		if (status === WalletConnectionStatus.Resetting && !connectingWallet && connectToWalletUponReset) {
	...
	return (
		<WalletManagerContext.Provider value={value}>
			...
			{status === WalletConnectionStatus.Resetting && (
				<BaseModal classNames={classNames} isOpen maxWidth="24rem" title="Resetting...">
					{renderLoader?.()}
				</BaseModal>
			)}
